...
- **页面工具条**（右下角，可收起）：关注 / 普通 / 跳过 计数，筛选「全部 / 只看关注 / 隐藏跳过」（Alt+F / Alt+H），「↓ 关注」跳到下一条关注（Alt+N），「重判」。筛选状态按标签页记在 sessionStorage（0.4.6 起开启时有页面内提示条）。
...
**0.3.0 追加（按「人怎么看信息」重做隐藏与突出）：**
- **跳过项默认折叠成一行**：只留小号标题 + 徽章（Scholar 隐藏作者行 / 摘要 / 操作行；arXiv 隐藏 dt 行与作者 / 评论 / 学科 / 摘要；PubMed 隐藏引用与摘要；X 限高两行）；连续 ≥2 条跳过合并成一行「▸ 已折叠 N 条跳过」，点击整段展开；单条双击展开。另有「变灰」「隐藏」两种方式，工具条下拉与设置页可切换，偏好持久化。
- **关注项突出**：左侧 3px 绿色色条 + 极淡绿底，眼睛滚动时先落到它；可在设置页关闭。
- **命中维度**：每篇多问 3 个 Noul（文献：课题 / 方法 / 材料；推文：主题 / 具体 / 来源），关注与普通条目在徽章旁显示「命中 课题 · 材料」，tooltip 给三维概率。可在设置页关闭（省约 25% token）。
- **把握不大**：三档最高概率 < 60% 时徽章改虚线边框，tooltip 注明。
- **关注置顶**：列表页工具条按钮（Alt+S）把 关注 → 普通 → 跳过 排到前面，只在同一父容器的连续区段内重排（arXiv 的「新投稿 / 交叉 / 替换」分区不混），再点恢复原顺序；推文时间线不提供。
- 工具条改为：计数 · 跳过显示方式下拉 · 只看关注 · 关注置顶 · ↓ · 重判 · ×；0.4.4 起为紧凑版（11px，闲置半透明）。

**0.4.0 追加：小红书 + Twitter 的隐藏与突出**
- **小红书网页版**（`xiaohongshu.com/explore`、搜索页、笔记弹层）：只看学习内容。独立「小红书画像」（读者概括 / 想学的主题 / 屏蔽的内容），推文式三级 Score（学习 / 一般 / 屏蔽）+ 「广告」Noul + 三维（主题 / 教学 / 干货）。瀑布流只有标题、封面、作者可见，判定主要靠中文标题（实测 14 / 14 命中）；打开笔记弹层后用正文全文重判并覆盖。
- **小红书重排的自愈**：监听站点对卡片 style 的改写（滚动加载 / 图片加载 / 缩放），识别为站点所为即重排；位置读取支持行内与 computed 两种；单条 / 弹层条目带 `data-pt-single`，不受筛选影响。
...
**0.4.6 追加：「只看关注」的状态可见与快捷键修复（Scholar 报告「时灵时不灵」）**
- **快捷键按物理键位判断**（`e.code`）：macOS 上 Option+F 送来的 `e.key` 是「ƒ」、Option+N / Option+H 是死键，按 `e.key` 比对的 Alt+F / Alt+N / Alt+S / Alt+H 在 Mac 上从未生效过；现在四个快捷键在 Mac 与 Windows 上一致可用。
- **只看关注开启时右上角常驻提示条**（`.pt-notice`，固定定位不碰页面布局）：「只看关注：显示 N 条关注 · 隐藏 M 条 · 判读中 K 条 · 未判读 E 条」，本页无关注项时明说「本页没有关注项」；带「显示全部」按钮一键退出。筛选状态按标签页记在 sessionStorage 并跨翻页 / 换搜索词保留，提示条让这个保留对用户可见。
- **未判读（出错）条目永不隐藏**：容器带 `data-pt-error`，只看关注与信息流站点的严格规则都排除它；工具条计数把「未判读」与「判读中」分开显示。
- 工具条上的「只看关注」激活态改为关注绿，与提示条呼应。
...
- 小红书只判标题（+ 作者名；弹层里加正文），不看封面图；无标题卡片不判。视频号 / 直播 / 广告位卡片若不是 `section.note-item` 结构则不处理。
...
- 待拍板：是否需要 Zotero 版本。
- 小红书的瀑布流重排是在未登录页面结构上推断的（登录后卡片才有真实位置）；如果登录后站点用别的定位方式，脚本会检测不到定位而退回「隐藏但不重排」（留洞）。需要在真机上看一眼。
- 折叠依赖各站点的 DOM 类名（写在 `badge.css` 的 `html.pt-skip-collapse.pt-site-*` 规则里），站点改版只影响折叠细节，不影响判读。
...
- 状态可见：判读中 / 未判读 / 三色结果都有徽章；tooltip 给概率与三维命中；把握不大用虚线；折叠处显示「已折叠 N 条」而不是凭空消失。
