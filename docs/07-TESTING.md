...
- 单元：50 / 50（新增：computed 位置读取、站点改写检测）。
- 端到端：小红书探索页新增「只看关注」断言（只剩关注卡片、重排后位置互不重叠、模拟站点把一张卡片挪到 (999,999) 后 500 ms 内被扩展挪回、关闭筛选后恢复）；新增笔记弹层页：只看关注时弹层标题仍显示。
- 仍未能在登录态的真实小红书上验证；弹窗「复制诊断信息」用于让用户把真实状态贴回来。
